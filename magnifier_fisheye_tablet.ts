...
export class MagnifierFisheyeTablet {
  ...
  public constructor(
    scene: THREE.Scene,
    texture_tablet: THREE.Texture,
    texture_sim: THREE.Texture
  ) {
    this.mesh = new THREE.Mesh(
      new THREE.PlaneGeometry(640 * 2, 480 * 2, 1, 1),
      new THREE.ShaderMaterial({
        ...
        void main()
        {
            vec2 myUv = vec2(vUv.x * 640. / 480. *2.0, vUv.y *2.0);
            vec2 diff = myUv - magnifier_center;
            if (length(diff) < 0.3) {
                vec2 newUv = magnifier_center + diff * 0.02;
                vec2 newNewUv = vec2(newUv.x / 640. * 480. -0.5, newUv.y -0.5);
        
                if (newNewUv.x < 0.0 || newNewUv.x >= 1.0 || newNewUv.y < 0.0 || newNewUv.y >= 1.0) {
                    discard;
                } else {
                    ...
                    if (new_vUv.x < 0.05 || new_vUv.x > 0.95 || new_vUv.y < 0.05 || new_vUv.y > 0.95) {
                        ...
                        } else if (simulation_result.b == 255) {
                        ...
                        } else if (simulation_result.r < 255) {
